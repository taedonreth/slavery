...
if __name__ == "__main__":
    ...
    numCourses1 = 2
    prereq1 = [[1, 0]]
    assert sol.canFinish(numCourses1, prereq1)
    ...
    numCourses3 = 5
    prereq3 = []
    assert sol.canFinish(numCourses3, prereq3)

    # Chain of dependencies, no cycle
    numCourses4 = 4
    prereq4 = [[1, 0], [2, 1], [3, 2]]
    assert sol.canFinish(numCourses4, prereq4)

    # Cycle in longer chain
    numCourses5 = 4
    prereq5 = [[1, 0], [2, 1], [0, 2]]
    assert not sol.canFinish(numCourses5, prereq5)

    # Disconnected components, no cycle
    numCourses6 = 6
    prereq6 = [[1, 0], [3, 2], [5, 4]]
    assert sol.canFinish(numCourses6, prereq6)

    # Disconnected components, one has a cycle
    numCourses7 = 6
    prereq7 = [[1, 0], [3, 2], [5, 4], [4, 5]]
    assert not sol.canFinish(numCourses7, prereq7)

    # Self dependency
    numCourses8 = 3
    prereq8 = [[0, 0]]
    assert not sol.canFinish(numCourses8, prereq8)

    # Large linear chain (stress test)
    numCourses9 = 1000
    prereq9 = [[i + 1, i] for i in range(999)]
    assert sol.canFinish(numCourses9, prereq9)
